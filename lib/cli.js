...
var _src_handlers_script_copy_js_CopyJsBuilder = {};
var _src_handlers_script_copy_js_CopyJsHandler = {};
var _src_handlers_script_copy_js_CopyJsParser = {};
var _src_handlers_script_copy_js_CopyJsRewriter = {};
var _src_handlers_script_data_DataInliner = {};
...
(function () {
    ...
    "use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.path_toAbsolute = exports.path_hasExtension = exports.path_changeExtension = exports.path_removeQuery = exports.path_sliceQuery = exports.path_sliceHash = exports.path_combine = exports.path_toRelative = exports.path_isRelative = exports.path_resolveUrl = exports.path_normalize = exports.path_resolveCurrent = exports.path_withProtocol = exports.path_appendQuery = exports.path_getExtension = exports.path_getFile = exports.path_getDir = void 0;
let isWeb = true;
...
exports.path_changeExtension = path_changeExtension;
...
let rgx_PROTOCOL = /^(file|https?):/i, rgx_SUB_DIR = /[^\/\.]+\/\.\.\//, rgx_FILENAME = /\/[^\/]+\.\w+(\?.*)?(#.*)?$/, rgx_EXT = /\.(\w+)$/, rgx_win32Drive = /(^\/?\w{1}:)(\/|$)/;
...
;
function async_resolve(result) {
    return (new atma_utils_1.class_Dfr()).resolve(result);
}
exports.async_resolve = async_resolve;
;
function async_reject(error) {
    return (new atma_utils_1.class_Dfr()).reject(error);
}
...
var Npm;
(function (Npm) {
    function resolveAppUrl(path, currentLocation, opts) {
        let base = opts.base;
        ...
        while (path != null) {
            ...
            if (atma_io_1.Directory.exists(dirname) === false) {
            ...
            if (path.startsWith('@') && resource.length > 0) {
                ...
                dirname = (0, path_1.path_combine)(dirname, `/${subPackage}`, '/');
                resource = resource.substring(subPackage.length + 1);
            }
            let pkg = (0, path_1.path_combine)(dirname, 'package.json');
            let json = global_1.io.File.read(pkg);
    ...
    function resolveAppUrlFromPackageJson(pkgDir, pkg, subPath, opts) {
        var _a;
        ...
        const conditionExportsMap = (_a = opts.package.platform) !== null && _a !== void 0 ? _a : 'node';
    ...
    Npm.resolveAppUrlFromPackageJson = resolveAppUrlFromPackageJson;
    function fromExports(exports, subPath, opts) {
        var _a;
        if (exports == null) {
        ...
        if (typeof exports === 'object') {
            ...
            for (let tpl in exports) {
                ...
                if (typeof mapped !== 'string') {
                    throw new Error(`Invalid conditional exports for ${JSON.stringify(exports)} in ${(_a = opts.pkgDir) !== null && _a !== void 0 ? _a : subPath}`);
                }
                if (tpl.startsWith('./')) {
                    tpl = tpl.substring(2);
                    if (tpl === '*') {
    ...
    function extractConditional(exports, opts) {
    ...
    function combineMain(directory, filename) {
        filename !== null && filename !== void 0 ? filename : (filename = 'index.js');
        let path = (0, path_1.path_combine)(directory, filename);
        if (hasExt(path) === false) {
            let likelyTs = path.includes('/src/');
            if (likelyTs) {
                if (global_1.io.File.exists(path + '.ts')) {
                    return path + '.ts';
                }
            }
            path += '.js';
...
class Resource {
    constructor(includeData, parent, solution) {
        ...
        let mappings = (_a = solution === null || solution === void 0 ? void 0 : solution.opts.mappings) !== null && _a !== void 0 ? _a : {};
        ...
        if (Include_1.Include.PathResolver.isNpm(includeData.url)) {
            ...
            let byNpmResolver = npm_1.Npm.resolveAppUrl(includeData.url, parent && parent.location, solution.opts);
        ...
        if (includeData.route != null) {
            ...
            if (path && Include_1.Include.PathResolver.isNpm(path) && this.aliases.includes(path) === false) {
                this.aliases.push(path);
            }
            if (alias && Include_1.Include.PathResolver.isNpm(alias) && this.aliases.includes(alias) === false) {
...
exports.FileActions = {
    readFile(path, opts) {
        ...
        return global_1.io.File.readAsync(path, {
            ...(opts !== null && opts !== void 0 ? opts : {}),
            cached: false
        });
    },
    async readDirectory(path, opts) {
        let i = path.indexOf('*');
        ...
        let files = await global_1.io.Directory.readFiles(directory, pattern);
...
class OutputItem {
    constructor(params) {
        ...
        this.resources = resources !== null && resources !== void 0 ? resources : [];
...
class BaseHandler {
    constructor(solution) {
        this.solution = solution;
        let { Parser, Rewriter, Builder, PathResolver } = this.constructor;
...
function findNode(node, fn, options = { scopeOnly: false }) {
...
function replaceWithVarIds(content, resource, solution) {
    ...
    return content.replace(rgx_REQUIRE, (full, path) => {
        let res = new Resource_1.Resource({ url: path, type: 'js' }, resource, solution);
...
class ModuleWrapper {
    ...
    wrapWithESM(body) {
        ...
        let rootContentRaw = global_1.io.File.read(rootPath, { cached: false, skipHooks: true, encoding: 'utf8' });
        let module = Parser_1.Parser.parse(rootContentRaw, new global_1.io.File(this.solution.path));
...
class CommonJsBuilder extends BaseScriptBuilder_1.BaseScriptBuilder {
    ...
    wrapModule(resource, outputItem, outputItems, setts) {
        ...
        let module = Templates_1.Templates
            .Module
            .replace('%MODULE_PATH%', () => `"${url}"`)
            .replace('%MODULE_ALIASES%', JSON.stringify(aliases !== null && aliases !== void 0 ? aliases : []))
    ...
    buildRoot(root, outputDependencies) {
        ...
        let body = outputDependencies
            ...
            .concat([content])
            .join('\n');
        let wrapper = new ModuleWrapper_1.ModuleWrapper(this.solution);
...
class CommonJsPathResolver extends BasePathResolver_1.BasePathResolver {
    ...
    resolve(includeData, parentResource) {
        let filename = npm_1.Npm.resolveAppUrl(includeData.url, parentResource.directory, this.solution.opts);
        if (filename) {
...
class IncludeJsParser extends BaseParser_1.BaseParser {
    ...
    getDependencies(ast, ownerResource) {
        ...
        AstUtil_1.AstUtil.each(ast, AstUtil_1.AstUtil.is.includeFunction, (node, descend) => {
            ...
            let scope = AstUtil_1.AstUtil.findNode(node, isIncludeSymbolRef).scope || ast;
    ...
    _processIncludeCallback(info, CallbackNode) {
        ...
        let meta = info.meta.includejs, args = CallbackNode.argnames, responseObjectName = args.length > 0
    ...
    _getPropertyAccessors(name, Fn) {
        ...
        Fn.body.forEach(function (x) {
            ...
            AstUtil_1.AstUtil.each(x, isSymbolName, function (node) {
                let chain = AstUtil_1.AstUtil.getPropertyChain(node, this.stack);
                if (chain) {
...
class ImportJsParser extends BaseParser_1.BaseParser {
    ...
    //     }
    // }
    async getDependencies(content, ownerResource) {
...
class ScriptParser extends BaseParser_1.BaseParser {
    ...
    getDependencies(content, ownerResource) {
        ...
        return (0, async_1.async_whenAll)(dfrs).then(results => {
            ...
            let merged = ResourceInfo_1.ResourceInfo.merge(...arr);
            return merged;
        });
    }
    accepts(type) {
...
(function () {
    // ensure AMD is not active for the model, so that any UMD exports as commonjs
    var define = null;
    var exports = _src_handlers_script_data_DataInliner != null ? _src_handlers_script_data_DataInliner : {};
    var module = { exports: exports };

    "use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.DataInliner = void 0;
const BaseRewriter_1 = _src_handlers_base_BaseRewriter;
class DataInliner extends BaseRewriter_1.BaseRewriter {
    rewriteRoot(resourceInput, resourceOutput) {
    }
    rewritePartial(content, ownerResource) {
    }
    async rewriteResource(resource) {
...
exports.DataInliner = DataInliner;
;
//# sourceMappingURL=DataInliner.js.map
//# sourceMappingURL=DataInliner.ts.map;

    function __isObj(x) {
        return x != null && typeof x === 'object' && x.constructor === Object;
    }
    if (_src_handlers_script_data_DataInliner === module.exports) {
        // do nothing if
    } else if (__isObj(_src_handlers_script_data_DataInliner) && __isObj(module.exports)) {
        Object.assign(_src_handlers_script_data_DataInliner, module.exports);
    } else {
        _src_handlers_script_data_DataInliner = module.exports;
    }
...
const IncludeJsHandler_1 = _src_handlers_script_include_js_IncludeJsHandler;
const BaseRewriter_1 = _src_handlers_base_BaseRewriter;
const DataInliner_1 = _src_handlers_script_data_DataInliner;
class ScriptRewriter extends BaseRewriter_1.BaseRewriter {
    constructor() {
        ...
        this.rewriters = [
            ...
            new DataInliner_1.DataInliner(this.solution, this.handler),
        ];
    }
    rewritePartial(content, ownerResource) {
    }
...
(function () {
    // ensure AMD is not active for the model, so that any UMD exports as commonjs
    var define = null;
    var exports = _src_handlers_script_copy_js_CopyJsParser != null ? _src_handlers_script_copy_js_CopyJsParser : {};
    var module = { exports: exports };

    "use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.CopyJsParser = void 0;
const BaseParser_1 = _src_handlers_base_BaseParser;
class CopyJsParser extends BaseParser_1.BaseParser {
    getDependencies(ast, ownerResource) {
        return null;
    }
}
exports.CopyJsParser = CopyJsParser;
;
//# sourceMappingURL=CopyJsParser.js.map
//# sourceMappingURL=CopyJsParser.ts.map;

    function __isObj(x) {
        return x != null && typeof x === 'object' && x.constructor === Object;
    }
    if (_src_handlers_script_copy_js_CopyJsParser === module.exports) {
        // do nothing if
    } else if (__isObj(_src_handlers_script_copy_js_CopyJsParser) && __isObj(module.exports)) {
        Object.assign(_src_handlers_script_copy_js_CopyJsParser, module.exports);
    } else {
        _src_handlers_script_copy_js_CopyJsParser = module.exports;
    }
...
(function () {
    // ensure AMD is not active for the model, so that any UMD exports as commonjs
    var define = null;
    var exports = _src_handlers_script_copy_js_CopyJsRewriter != null ? _src_handlers_script_copy_js_CopyJsRewriter : {};
    var module = { exports: exports };

    "use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.CopyJsRewriter = void 0;
const BaseRewriter_1 = _src_handlers_base_BaseRewriter;
class CopyJsRewriter extends BaseRewriter_1.BaseRewriter {
    rewritePartial(content, ownerResource) {
    }
    rewriteResource(resource) {
    }
...
exports.CopyJsRewriter = CopyJsRewriter;
;
//# sourceMappingURL=CopyJsRewriter.js.map
//# sourceMappingURL=CopyJsRewriter.ts.map;

    function __isObj(x) {
        return x != null && typeof x === 'object' && x.constructor === Object;
    }
    if (_src_handlers_script_copy_js_CopyJsRewriter === module.exports) {
        // do nothing if
    } else if (__isObj(_src_handlers_script_copy_js_CopyJsRewriter) && __isObj(module.exports)) {
        Object.assign(_src_handlers_script_copy_js_CopyJsRewriter, module.exports);
    } else {
        _src_handlers_script_copy_js_CopyJsRewriter = module.exports;
    }
...
(function () {
    // ensure AMD is not active for the model, so that any UMD exports as commonjs
    var define = null;
    var exports = _src_handlers_script_copy_js_CopyJsBuilder != null ? _src_handlers_script_copy_js_CopyJsBuilder : {};
    var module = { exports: exports };

    "use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.CopyJsBuilder = void 0;
 ...
 */
class CopyJsBuilder extends BaseScriptBuilder_1.BaseScriptBuilder {
    wrapModule(resource, outputItem) {
        let { content, dependencies } = resource;
        let offset = 0;
        dependencies && dependencies.filter(x => x.pos != null && (0, path_1.path_isRelative)(x.url)).forEach(dep => {
    ...
    async buildRoot(root, outputDependencies) {
        let outputResources = this.solution.outputResources;
        outputResources.items = await (0, alot_1.default)(outputResources.items)
            .mapManyAsync(async (item) => {
            if (item.resource.isGlob !== true) {
                return [item];
            }
            // Normal output
            let copyItems = await (0, alot_1.default)(item.resources)
                .mapMany(resource => {
    ...
    async createOutputItems(parent, resource) {
        var _a;
        let solution = this.solution;
        let rewriteToRelativeUrls = {};
        if (((_a = resource.dependencies) === null || _a === void 0 ? void 0 : _a.length) > 0) {
            let appBase = this.solution.opts.base;
            (0, alot_1.default)(resource.dependencies)
                .forEach(dep => {
                ...
                let relativePath = (0, path_1.path_toRelative)(dep.resource.filename, resource.filename);
                if (relativePath == null || /(..\/)*?\/?node_modules/.test(relativePath)) {
                    return;
                }
                relativePath = (0, path_1.path_changeExtension)(relativePath, this.solution.opts.outputExtension);
        ...
        let readFile = Configuration_1.Configuration.Instance.get('readFile');
        await (0, alot_1.default)(extra).forEachAsync(async (x) => {
            ...
            result.push(createOutputItem(cloneWithExtension(cloned, x.output)));
            if (x.ext === 'ts.map' && cloned.content != null) {
                ...
                let rel = (0, path_1.path_toRelative)(source, target);
                outputItem.resource.content = outputItem.resource.content.replace(/"sources":\["[^"]+"\]/, `"sources":["${rel}"]`);
            }
        }).toArrayAsync({ threads: 1 });
        function createOutputItem(resource) {
            return new OutputResources_1.OutputItem({
                page: parent.page,
        ...
        function cloneWithExtension(resource, ext) {
            let cloned = resource.clone();
            cloned.filename = (0, path_1.path_changeExtension)(cloned.filename, ext);
            cloned.url = (0, path_1.path_changeExtension)(cloned.url, ext);
            return cloned;
        }
        function rewriteUrls(resource, rewriteToRelativeUrls) {
            let content = resource.content;
            if (content == null) {
...
exports.CopyJsBuilder = CopyJsBuilder;
;
//# sourceMappingURL=CopyJsBuilder.js.map
//# sourceMappingURL=CopyJsBuilder.ts.map;

    function __isObj(x) {
        return x != null && typeof x === 'object' && x.constructor === Object;
    }
    if (_src_handlers_script_copy_js_CopyJsBuilder === module.exports) {
        // do nothing if
    } else if (__isObj(_src_handlers_script_copy_js_CopyJsBuilder) && __isObj(module.exports)) {
        Object.assign(_src_handlers_script_copy_js_CopyJsBuilder, module.exports);
    } else {
        _src_handlers_script_copy_js_CopyJsBuilder = module.exports;
    }
...
(function () {
    // ensure AMD is not active for the model, so that any UMD exports as commonjs
    var define = null;
    var exports = _src_handlers_script_copy_js_CopyJsHandler != null ? _src_handlers_script_copy_js_CopyJsHandler : {};
    var module = { exports: exports };

    "use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.CopyJsHandler = void 0;
const BaseHandler_1 = _src_handlers_base_BaseHandler;
const CopyJsParser_1 = _src_handlers_script_copy_js_CopyJsParser;
const CopyJsRewriter_1 = _src_handlers_script_copy_js_CopyJsRewriter;
const CopyJsBuilder_1 = _src_handlers_script_copy_js_CopyJsBuilder;
class CopyJsHandler extends BaseHandler_1.BaseHandler {
}
exports.CopyJsHandler = CopyJsHandler;
CopyJsHandler.Parser = CopyJsParser_1.CopyJsParser;
CopyJsHandler.Rewriter = CopyJsRewriter_1.CopyJsRewriter;
CopyJsHandler.Builder = CopyJsBuilder_1.CopyJsBuilder;
CopyJsHandler.PathResolver = null;
;
//# sourceMappingURL=CopyJsHandler.js.map
//# sourceMappingURL=CopyJsHandler.ts.map;

    function __isObj(x) {
        return x != null && typeof x === 'object' && x.constructor === Object;
    }
    if (_src_handlers_script_copy_js_CopyJsHandler === module.exports) {
        // do nothing if
    } else if (__isObj(_src_handlers_script_copy_js_CopyJsHandler) && __isObj(module.exports)) {
        Object.assign(_src_handlers_script_copy_js_CopyJsHandler, module.exports);
    } else {
        _src_handlers_script_copy_js_CopyJsHandler = module.exports;
    }
...
const ImportJsHandler_1 = _src_handlers_script_import_js_ImportJsHandler;
const CopyJsHandler_1 = _src_handlers_script_copy_js_CopyJsHandler;
class ScriptBuilder extends BaseBuilder_1.BaseBuilder {
    constructor() {
        super(...arguments);
        this.builders = [
            new CopyJsHandler_1.CopyJsHandler.Builder(this.solution, this.handler),
...
const global_1 = _src_global;
const shellbee_1 = _node_modules_shellbee_lib_shellbee;
class Solution extends atma_utils_1.class_EventEmitter {
    ...
    runScripts(name, ...args) {
        return (0, async_1.async_run)((resolve, reject) => {
            ...
            (0, async_1.async_waterfall)(arr, async function (pathOrCommand) {
                if (/^[.\/]/.test(pathOrCommand)) {
                    let mix = require(atma_utils_1.class_Uri.combine(process.cwd(), pathOrCommand));
                    if (mix && mix.process) {
                        return mix.process();
                    }
                }
                else {
                    let result = await (0, shellbee_1.run)(pathOrCommand);
                }
...
function getDependenciesInternal(resource, solution) {
    ...
    if (resource.isGlob) {
        ...
        return (0, async_1.async_resolve)(result);
    }
    let ext = (0, path_1.path_getExtension)(resource.url);
    ...
    function load(includeData, parent, opts, solution) {
        ...
        if (loader == null) {
            ...
            if (resource.aliases.length > 0) {
                loader.resource.aliases = (0, alot_1.default)([
    ...
    class TreeLoader {
        ...
        async process() {
            ...
            try {
                ...
                let reader = this.resource.isGlob
                    ? Configuration_1.Configuration.Instance.get('readDirectory')
                    : Configuration_1.Configuration.Instance.get('readFile');
                ...
                if (((_a = handler === null || handler === void 0 ? void 0 : handler.parser) === null || _a === void 0 ? void 0 : _a.transpile) != null) {
                    let result = await handler.parser.transpile(content, this.resource);
                    content = (_b = result === null || result === void 0 ? void 0 : result.content) !== null && _b !== void 0 ? _b : content;
                }
                let end = Date.now();
        ...
        processChildren() {
            ...
            Parser_1.Parser
                ...
                .then(result => {
                var _a;
                this.solution.reporter.print((0, color_1.color)(` cyan<${(_a = result.dependencies) === null || _a === void 0 ? void 0 : _a.length}> dep\n`));
...
const res_1 = _src_utils_res;
const atma_utils_1 = require("atma-utils");
function tree_async(workerInfo) {
    let start = Date.now();
    let dfr;
    try {
        ...
        console.error(error);
        return atma_utils_1.class_Dfr.reject(error);
    }
...
exports.Builder = {
    build(resources, solution) {
        return Middlewares_1._middlewares
            ...
            .then(arr => {
            resources = arr !== null && arr !== void 0 ? arr : resources;
        ...
        async function rewriteDependenciesInternal(resources) {
            await (0, alot_1.default)(resources).forEachAsync(async (resource, i) => {
                let ext = (0, path_1.path_getExtension)(resource.url);
                let handler = solution.handlers.find(x => x.rewriter.accepts(ext));
                if (handler == null) {
        ...
        function rewriteRoot() {
            return (0, async_1.async_run)(() => {
                ...
                let ext = (0, path_1.path_getExtension)(input.url);
        ...
        function buildOutputItems() {
            ...
            return (0, alot_1.default)(items).mapAsync(item => {
        ...
        function buildBundleInternal(outputItem, otherOutputItems) {
            ...
            let ext = (0, path_1.path_getExtension)(outputItem.resource.url);
        ...
        function buildRoot() {
            var _a;
            let main = (_a = solution.outputResources.rootOutput) !== null && _a !== void 0 ? _a : solution.outputResources.rootInput;
            let dependencies = solution.outputResources.getForPage(solution.opts.mainPage);
            let ext = (0, path_1.path_getExtension)(main.url);
...
class Bundler extends atma_utils_1.class_EventEmitter {
    ...
    build(opts) {
        ...
        function build(resource) {
            isBuilding = true;
            let resources = (0, res_1.res_flattern)(resource, solution.opts.dependencies);
            return (0, tree_1.tree_async)({
                ...
                message: (treeInfo, seconds) => {
                    return (0, color_1.color)(`Created bold<yellow<${treeInfo.count}>> files in bold<yellow<${seconds}>> sec.`);
                }
            })
                .done(buildComplete)
                .fail(buildFailed);
    ...
    static process(path, opts) {
        ...
        function builderComplete(resources) {
            resources.forEach(res => {
                ...
                File_1.FileActions.writeFile(res.filename, content, { skipHooks: false });
            });
            return solution
                .assetsManager
                .flush()
        ...
        return {
            getDependencies(content, opts = { type: 'js' }) {
                if (typeof opts === 'string')
                    opts = { type: opts };
                let solution = new Solution_1.Solution('', opts);
                let resource = new Resource_1.Resource({ type: opts.type, content: content }, null, solution);
