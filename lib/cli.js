...
function res_getTreeInfo(mix) {
    ...
    var paths = arr.map(x => {
        var pages = x.inPages.map(page => color_1.color(`bg_white<black<${page}>>`)).join(' ');
        return `${x.url} ${pages}`;
    }).sort();
    return {
        count: arr.length,
...
(function (Npm) {
    function resolveAppUrl(path, currentLocation, base) {
        ...
        while (path != null) {
            let dirname = path_1.path_combine(currentLocation, 'node_modules', name);
            let pckg = path_1.path_combine(dirname, 'package.json');
...
class Resource {
    ...
    _toStaticTarget(solution, settings) {
        ...
        let url = this.url;
        let filename = path_1.path_removeQuery(url);
    ...
    _toOutputTarget(solution, settings) {
        ...
        let filename = path_1.path_combine(opts.outputBase, url);
    ...
    toRelative(resource) {
        let url = path_1.path_toRelative(this.filename, resource.filename);
        return url;
    }
...
class AssetsManager {
    ...
    rewriteAssets(resource, targetResource, solution) {
        ...
        if (type === 'load') {
            let ext = path_1.path_getExtension(resource.url);
...
class SolutionOpts extends SolutionOptsBase {
    constructor(solution, opts_) {
        ...
        for (let key in this.defaults) {
            this[key] = obj_1.obj_deepDefaults(opts[key], this.defaults[key]);
        }
        for (let key in this.resolvers) {
            this[key] = this.resolvers[key].call(this, this[key], this);
        }
        if (!this.type && solution.path) {
            this.type = this.getTypeForExt(path_1.path_getExtension(solution.path));
        }
    }
    ...
    prepair(resources) {
        ...
        Object.keys(this.pagesInput).forEach(page => {
            Object.keys(this.pagesInput[page]).forEach(bundle => {
                ...
                let byType = res_1.res_groupResourcesByType(resources, this.solution.opts);
...
class MaskRewriter extends BaseRewriter_1.BaseRewriter {
    rewritePartial(content, ownerResource) {
        ...
        global_1.mask.TreeWalker.walk(ast, node => {
            ...
            if (ownerResource.source && ownerResource.location !== ownerResource.source.location) {
                node
                    .nodes
                    .filter(x => path_1.path_isRelative(x.path))
                    .forEach(x => {
                    let ownerSource = ownerResource.source.url, ownerTarget = ownerResource.url, currentUrl = path_1.path_normalize(path_1.path_combine(path_1.path_getDir(ownerSource), x.path)), targetUrl = path_1.path_toRelative(currentUrl, ownerTarget);
    ...
    rewriteRoot(resourceInput, resourceOutput) {
        ...
        global_1.mask.TreeWalker.walk(ast, (node) => {
            ...
            let path = path_1.path_combine(path_1.path_getDir(resourceInput.url), node.path);
...
class IncludeJsBuilder extends BaseScriptBuilder_1.BaseScriptBuilder {
    wrapScriptlessModule(otherOutputItems) {
        var allResources = arr_1.arr_flattern(otherOutputItems.map(x => x.resources));
    ...
    _createHeading(builderOpts, resource, outputItem, otherOutputItems) {
        ...
        var allResources = arr_1.arr_flattern(outputItems.map(x => x.resources));
        var jsResources = allResources.filter(x => this.accepts(x));
        var cssResources = allResources.filter(x => x.type === 'css');
        var loadResources = allResources.filter(x => x.type === 'load');
...
class ModuleFile {
    ...
    toScript(parents = [], options, indent = '') {
        let newLine = utils_1.u_getNewLine(this.content, io);
        ...
        const SPACE = '\t';
        let indentScopedContent = `${utils_1.u_makeIndent(scopedContent || '', SPACE, io)}`;
        let indentContent = `${utils_1.u_makeIndent(content, SPACE, io)}`;
...
class ScriptParser extends BaseParser_1.BaseParser {
    ...
    getDependencies(content, ownerResource) {
        ...
        return async_1.async_whenAll(dfrs).then(results => {
            let arr = arr_1.arr_flattern(results);
...
class GlobalJsBuilder extends BaseScriptBuilder_1.BaseScriptBuilder {
    wrapModule(resource, outputItem) {
        ...
        let offset = 0;
        dependencies && dependencies.filter(x => x.pos != null && path_1.path_isRelative(x.url)).forEach(dep => {
...
class HtmlParser extends BaseParser_1.BaseParser {
    ...
    getDependencies(content, ownerResource) {
        ...
        return async_1.async_map(queue, x => x.reader.read(x.node))
...
class Solution extends atma_utils_1.class_EventEmitter {
    ...
    runScripts(name, ...args) {
        return async_1.async_run((resolve, reject) => {
...
exports.Loader = {
    ...
    load(type, path, opts, solution) {
        ...
        return ResourceLoader
            ...
            let treeInfo = res_1.res_getTreeInfo(loader.resource);
    ...
    getTypeFromPath(path) {
        let ext = path_1.path_getExtension(path);
    ...
    class TreeLoader {
        ...
        process() {
            ...
            function toMessage(path) {
                ...
                let name = parts.pop();
                parts = parts.map(x => color_1.color(`bold<${x}>`));
            ...
            reader(this.resource.filename, this.opts).then(content => {
                let end = Date.now();
                this.solution.reporter.print(color_1.color(` cyan<${end - start}> ms \n`));
        ...
        async loadChildren(result) {
            ...
            const resources = await alot_1.default(deps).mapAsync(async (dep) => {
...
class WatcherFactory extends atma_utils_1.class_EventEmitter {
    ...
    changed(filename) {
        ...
        var resource = res_1.res_find(this.rootResource, (res) => res.filename === filename);
        if (resource == null)
...
(function () {
    exports.tree_async = tree_async = function (workerInfo) {
        ...
        dfr.done(resources => {
            let end = Date.now();
            let seconds = ((end - start) / 1000).toFixed(2);
            let treeInfo = res_1.res_getTreeInfo(resources);
            reporter
...
class Bundler extends atma_utils_1.class_EventEmitter {
    ...
    getResources(opts) {
        ...
        return Loader_1.Loader.load(type, path, opts, solution).then(resource => {
            return res_1.res_flattern(resource, solution.opts.dependencies).map(x => x);
        });
    }
    ...
    build(opts) {
        ...
        function build(resource) {
            ...
            return tree_1.tree_async({
                ...
                message: (treeInfo, seconds) => color_1.color(`Created bold<yellow<${treeInfo.count}>> files in bold<yellow<${seconds}>> sec.`)
...
function ensureBundlesConfig(config) {
    if (isMultiConfig(config)) {
        ...
        let bundles = Object.keys(config.apps)
            ...
            .map(name => {
            let obj = { name };
            obj_1.obj_deepExtend(obj, JSON.parse(JSON.stringify(common)));
...
(function (Runner) {
    function run(config, bundleFn = mainBundlerFunction) {
        let bundles = MultiBundle_1.ensureBundlesConfig(config);
